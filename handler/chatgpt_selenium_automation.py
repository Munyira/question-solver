...
class ChatGPTAutomation:
    ...
    def launch_chrome_with_remote_debugging(self, port, url):
        """Launches Chrome with remote debugging enabled (Windows-compatible)"""
        def open_chrome():
            user_data_dir = os.path.join(os.getcwd(), "temporarychromeuser")
            os.makedirs(user_data_dir, exist_ok=True)

            chrome_cmd = f'"{self.chrome_path}" --remote-debugging-port={port} --user-data-dir="{user_data_dir}" "{url}"'
            print("Running Chrome command:", chrome_cmd)  # optional for debugging
